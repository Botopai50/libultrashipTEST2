...
float4 PSMain(PSInput input, float4 screenSpace : SV_Position) : SV_TARGET {
    ...
    @if(o_water)
    {
        ...
        // The composed colour already contains what is behind the surface, so it must land on the frame
        // as-is rather than being blended over that same background a second time. Alpha goes to one --
        // except in the last few units at the shoreline, where it eases off so the waterline meets the sand
        // as a soft edge instead of a cut (2.19, item 11).
